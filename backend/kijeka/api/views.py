...
def getCategoryProducts(request):
    if request.method == 'GET':
        ...
        productArr = []
        for product in productList:
            productArr.append([product.productName,product.description,"/media/images/"+(product.images.url).split("/")[-1]])
        if len(productArr) == 0:
...
def dataUpdater(request):
    ...
    for product in products:
        product.description = product.description.replace("rgb(134, 134, 134)","rgb(0, 75, 149)")
        product.save()
