...
const AllProducts = () => {
	...
	if (products.length === 0) {
		axios
			.get("http://" + process.env.REACT_APP_SERVER + "/api/productList/")
			.then((data) => setProducts(data["data"]["data"]))
			.catch((error) => console.log(error));
	}
	document.title = "Kijeka | All Products";
	document.getElementById("header_title").innerHTML = "All Products";
		...
		<div className="w-full min-h-screen bg-grayBG md:p-8 p-4">
			<div className="flex flex-col md:flex-row md:items-center md:justify-between">
				...
				<div className="flex flex-col md:flex-row md:items-center my-2 md:my-0 md:gap-7">
					<p className="text-base font-bold text-kijeka-blue/40 underline">Copy and add new product</p>
					...
					<Link to={`/add-products`} className="text-sm font-bold text-kijeka-blue underline w-fit">
						Add Products To Hot Product
					</Link>
				</div>
			</div>
